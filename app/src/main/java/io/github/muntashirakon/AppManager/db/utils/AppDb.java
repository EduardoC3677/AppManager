...
public class AppDb {
    ...
    public List<App> getAllInstalledApplications() {
        synchronized (sLock) {
            return mAppDao.getAllInstalledSync();
        }
    }
    ...
    private List<App> updateApplicationInternal(@NonNull Context context, @NonNull String packageName) {
        ...
        List<App> oldApps = new ArrayList<>(mAppDao.getAllSync(packageName));
        List<App> appList = new ArrayList<>(userIds.length);
        List<Backup> backups = new ArrayList<>(mBackupDao.getSync(packageName));
        for (int userId : userIds) {
            ...
            if (backup == null && packageInfo == null) {
                ...
                if (oldAppIndex >= 0) {
                    ...
                    mAppDao.deleteSync(oldApps.get(oldAppIndex));
                }
                continue;
            }
            if (oldAppIndex >= 0) {
    ...
    public void updateApplications(@NonNull Context context) {
        synchronized (sLock) {
            ...
            List<App> oldApps = new ArrayList<>(mAppDao.getAllSync());
