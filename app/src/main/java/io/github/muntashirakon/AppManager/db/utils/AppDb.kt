...
class AppDb {
    ...
    fun getAllInstalledApplications(): List<App> {
        synchronized(sLock) {
            return mAppDao.allInstalledSync
        }
    }

    fun getAllApplications(packageName: String): List<App> {
        synchronized(sLock) {
            return mAppDao.getAllSync(packageName)
        }
    }

    fun getAllApplications(packageName: String, userId: Int): List<App> {
        synchronized(sLock) {
            return mAppDao.getAllSync(packageName, userId)
        }
    }

    fun getAllBackups(): List<Backup> {
        synchronized(sLock) {
            return mBackupDao.allSync
        }
    }
    ...
    private fun updateApplicationInternal(context: Context, packageName: String): List<App> {
        ...
        val appList: MutableList<App> = ArrayList(userIds.size)
        ...
        for (userId in userIds) {
            ...
            try {
                packageInfo = PackageManagerCompat.getPackageInfo(
                    packageName,
        ...
        synchronized(sLock) {
            ...
            for (userId in Users.getUsersIds()) {
                ...
                for (packageInfo in packageInfoList) {
                    ...
                    val oldAppIndex = findIndexOfApp(
                        oldApps,
    ...
    companion object {
        ...
        private fun updateVariableData(context: Context, modifiedApps: List<App>) {
            ...
            val packageUsageInfoList: MutableList<PackageUsageInfo> = ArrayList()
        ...
        private fun isUpToDate(currentApp: App, installedPackageInfo: PackageInfo): Boolean {
